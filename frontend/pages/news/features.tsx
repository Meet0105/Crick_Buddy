...
export default function Features({ news }: { news: NewsItem[] }) {
  return (
    <div className="bg-gray-900 min-h-screen w-full overflow-x-hidden">
      ...
      <main className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-6 sm:py-8 w-full">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 sm:mb-8 lg:mb-10 gap-3 sm:gap-4">
          <h1 className="text-xl sm:text-2xl lg:text-3xl font-black text-green-400">Features</h1>
        ...
        {news && news.length > 0 ? (
          ...
          <div className="bg-gray-800/95 backdrop-blur-sm rounded-2xl sm:rounded-3xl shadow-2xl p-8 sm:p-10 md:p-12 text-center border border-gray-700">
            ...
            <h3 className="text-lg sm:text-xl md:text-2xl font-bold text-gray-100 mb-3 sm:mb-4 px-2">No Features Available</h3>
            <p className="text-sm sm:text-base text-gray-400 mb-6 sm:mb-8 px-2">Feature articles are currently being synced. Please check back later!</p>
...
export async function getServerSideProps() {
  try {
    ...
    // Fetch news and filter for features
    ...
    // Filter for features
    const features = allNews.filter((item: NewsItem) => 
      item.category === 'FEATURES' || 
      item.category === 'ANALYSIS' ||
      item.category === 'OPINION' ||
      item.headline.toLowerCase().includes('feature') ||
      item.headline.toLowerCase().includes('analysis')
    );
    return { props: { news: features } };
  } catch (error) {
