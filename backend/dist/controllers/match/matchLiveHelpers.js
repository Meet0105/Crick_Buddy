...
exports.processRawMatchData = processRawMatchData;
exports.extractMatchInfoFromFlatStructure = extractMatchInfoFromFlatStructure;
exports.extractMatchInfo = extractMatchInfo;
exports.extractScoresFromScorecard = extractScoresFromScorecard;
// Helper function to map API status to our enum values
const mapStatusToEnum = (status) => {
    if (!status)
        return 'LIVE'; // For live helpers, default to LIVE if no status
...
function processRawMatchData(match) {
...
function extractMatchInfoFromFlatStructure(rawData, originalMatch) {
    var _a;
    ...
    if (matchScore && Object.keys(matchScore).length > 0) {
        ...
        const calculateTotalScore = (teamScoreData) => {
            let totalRuns = 0;
            let totalWickets = 0;
            let totalOvers = 0;
            let totalBalls = 0;
    ...
    if (rawData.startdate)
        startDate = new Date(parseInt(rawData.startdate));
    else if (rawData.startDate)
        startDate = new Date(parseInt(rawData.startDate));
    else if (originalMatch.startDate)
        startDate = new Date(originalMatch.startDate);
    let endDate = null;
    if (rawData.enddate)
        endDate = new Date(parseInt(rawData.enddate));
    else if (rawData.endDate)
        endDate = new Date(parseInt(rawData.endDate));
    else if (originalMatch.endDate)
        endDate = new Date(originalMatch.endDate);
    ...
    return {
        ...originalMatch,
        matchId: ((_a = (rawData.matchid || rawData.matchId)) === null || _a === void 0 ? void 0 : _a.toString()) || originalMatch.matchId,
        format,
        title,
...
function extractMatchInfo(matchInfo, matchScore, originalMatch) {
    ...
    const venueName = ((_a = matchInfo.venueInfo) === null || _a === void 0 ? void 0 : _a.ground) || ((_b = matchInfo.venueInfo) === null || _b === void 0 ? void 0 : _b.stadium) || matchInfo.venue || 'Venue TBD';
    const venueCity = ((_c = matchInfo.venueInfo) === null || _c === void 0 ? void 0 : _c.city) || '';
    const venueCountry = ((_d = matchInfo.venueInfo) === null || _d === void 0 ? void 0 : _d.country) || '';
    let startDate = null;
    if (matchInfo.startDate)
        startDate = new Date(parseInt(matchInfo.startDate));
    else if (matchInfo.startDateTime)
        startDate = new Date(matchInfo.startDateTime);
    else if (originalMatch.startDate)
        startDate = new Date(originalMatch.startDate);
    let endDate = null;
    if (matchInfo.endDate)
        endDate = new Date(parseInt(matchInfo.endDate));
    else if (matchInfo.endDateTime)
        endDate = new Date(matchInfo.endDateTime);
    else if (originalMatch.endDate)
        endDate = new Date(originalMatch.endDate);
    // Extract result information if available (for live matches that might be completed)
    ...
    if (matchInfo.liveText || matchInfo.commentary) {
        commentary = {
            liveText: matchInfo.liveText || ((_e = matchInfo.commentary) === null || _e === void 0 ? void 0 : _e.text) || '',
    ...
    if (matchInfo.currBatTeamId) {
        currentlyPlaying = {
            teamId: (_f = matchInfo.currBatTeamId) === null || _f === void 0 ? void 0 : _f.toString(),
    ...
    return {
        ...originalMatch,
        matchId: ((_g = matchInfo.matchId) === null || _g === void 0 ? void 0 : _g.toString()) || originalMatch.matchId,
        format,
        title,
    ...
    inningsData.forEach((innings, index) => {
        ...
        if (batsmen && typeof batsmen === 'object') {
            ...
            if (potentialBatsmenKeys.length > 0) {
                ...
                if (potentialBatsmenKeys.length > 0) {
                    const firstBatsman = batsmen[potentialBatsmenKeys[0]];
        ...
        if (batsmen && !Array.isArray(batsmen)) {
            ...
            if (batsmen.batsman && Array.isArray(batsmen.batsman)) {
            ...
            // Or convert object values to array if it's a key-value object
                ...
                const batsmenValues = Object.values(batsmen);
        ...
        if (runs === 0 && (inningsData.batsmen || inningsData.batsman)) {
            ...
            if (batsmen && !Array.isArray(batsmen)) {
                ...
                // Check if we should use the inningsData.batsman field directly
                    ...
                    const validBatsmen = batsmanEntries.filter(([key, value]) => {
                        ...
                        if (isValid) {
                            console.log(`Innings ${index + 1}: Valid batsman entry [${key}]:`, value.runs, 'runs');
                        }
                        return isValid;
                    }).map(([key, value]) => value);
            ...
            if (Array.isArray(batsmen) && batsmen.length > 0) {
                ...
                runs = batsmen.reduce((total, batsman) => {
                    const batsmanRuns = (batsman === null || batsman === void 0 ? void 0 : batsman.runs) || 0;
